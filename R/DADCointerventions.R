#' DADCointerventions returns the cointerventions for a given treatment.
#'
#' @param treatment
#' @param spark_data_frame
#'
#' @return cointerventions as data table
#' @export
#'
#' @examples
#' r_dt <- DADCointerventions("I")
DADCointerventions <- function(treatment, spark_data_frame = NULL){
  if(!is.null(spark_data_frame)){
    createOrReplaceTempView(spark_data_frame, "spark_data_frame")
  }

  patient_set = DADSameTreatment(treatment)
